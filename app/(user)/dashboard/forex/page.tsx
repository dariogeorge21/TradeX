...
import { PopularForexPairs } from "@/components/forex/PopularForexPairs";
import { Search } from "lucide-react";
import { MotionDiv } from "@/components/ui/motion-wrapper";
import { getForexMarketSnapshots } from "@/services/forex-research";

export const metadata: Metadata = {
  title: "Forex — TradeX",
  description: "AI-powered forex research and intelligence.",
};

const DEFAULT_PAIRS = "EURUSD,GBPUSD,USDJPY,AUDUSD,USDCAD,USDCHF,NZDUSD,EURGBP,EURJPY,GBPJPY";

export default async function ForexIndexPage() {
  const snapshots = await getForexMarketSnapshots(DEFAULT_PAIRS);
  ...
  return (
    ...
    >
      ...
      <Card className="relative overflow-hidden border-2 border-violet-500/20 bg-card/80 backdrop-blur-xl shadow-2xl shadow-violet-500/5">
        ...
        <CardHeader className="relative text-center pt-10">
          ...
          <p className="text-muted-foreground max-w-xl mx-auto mt-2">
            Search any currency pair to access an institutional-grade dashboard with real-time AI synthesis.
          </p>
        </CardHeader>
      ...
      {/* Popular Forex Pairs Section */}
      <div className="space-y-4">
        <h2 className="text-xl font-bold tracking-tight">Popular Currency Pairs</h2>
        <PopularForexPairs snapshots={snapshots} />
      </div>
    </MotionDiv>
  );
}
