...
export function CryptoAISummaryCard({
  ticker,
  meta,
}: {
  ticker: string;
  meta: CryptoSnapshotTicker;
}) {
  ...
  React.useEffect(() => {
    ...
    async function fetchStream() {
      try {
        const res = await fetch(`/api/crypto/${encodeURIComponent(ticker)}/ai-summary`);
    ...
    <div className="rounded-2xl border border-amber-500/30 bg-amber-500/5 p-6 shadow-sm">
      ...
      {loading ? (
        <div className="space-y-3 animate-pulse">
          ...
          <div className="h-4 bg-amber-500/20 rounded w-4/5"></div>
        </div>
      ) : error ? (
