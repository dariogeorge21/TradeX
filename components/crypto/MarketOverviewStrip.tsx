...
import { TrendingDown, TrendingUp, Bitcoin, Loader2 } from "lucide-react";
import { CryptoSnapshotTicker } from "@/types/crypto";
import { formatCurrency, formatPercentage, cn } from "@/lib/utils";
import { MotionDiv } from "@/components/ui/motion-wrapper";

function MarketOverviewItem({ snap, index }: { snap: CryptoSnapshotTicker; index: number }) {
  ...
  const handleNavigate = () => {
    startTransition(() => {
      router.push(`/dashboard/crypto/${encodeURIComponent(snap.ticker)}`);
    });
  };
    ...
    >
      <button
        ...
        className={cn(
          ...
          "hover:-translate-y-1 hover:shadow-lg hover:border-amber-500/30 hover:bg-card/80 text-left",
          isPending && "opacity-70 pointer-events-none"
        )}
      >
        <div className="flex items-center justify-between mb-3 w-full">
          <div className="flex items-center gap-2">
            <div className="rounded-full bg-amber-500/20 p-1.5 text-amber-500 transition-transform group-hover:scale-110">
        ...
        <div className="text-xl font-bold tracking-tight flex items-center justify-between w-full">
          <span>{formatCurrency(snap.day.c)}</span>
          {isPending && <Loader2 className="h-4 w-4 animate-spin text-amber-500" />}
        </div>
      </button>
    </MotionDiv>
  );
...
export function MarketOverviewStrip({
  ...
  const content = displaySnaps.map((snap, index) => (
    <MarketOverviewItem key={snap.ticker} snap={snap} index={index} />
  ));
